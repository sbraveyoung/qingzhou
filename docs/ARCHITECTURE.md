...
└────────────────────┬────────────────────────────────────┘
                     │ 依赖
┌────────────────────▼────────────────────────────────────┐
│ VPNApp  (SwiftUI 视图 + AppState 协调器)                │
└────┬────────┬────────┬─────────┬────────┬──────────┬────┘
     │        │        │         │        │          │
     ▼        ▼        ▼         ▼        ▼          ▼
┌────────┐┌─────────────┐┌─────────┐┌──────────┐┌─────────┐
│VPNCore ││VPNProtocols ││VPNRules ││VPNSpeed- ││VPNLog-  │
...
依赖方向只能从上往下；同层之间不互相依赖。`VPNCore` 是唯一被所有模块引入的基础库。
...
- **重要类型**：
  ...
  - `Subscription`：流量 / 到期由 `SubscriptionUserInfo` 在 VPNSubscription 写入。
...
1. **集成 sing-box 作为协议核心**。把 sing-box 编成 xcframework：
   ...
   ```
   把产物放到 `Frameworks/SingBox.xcframework`，新建 `VPNCore-SingBox` 模块作为 Swift 与 Go 的胶水层。
