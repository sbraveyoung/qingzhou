...
或者用 Xcode：
1. `xcodegen generate && open VPN.xcodeproj`
2. 选 `VPN-macOS` scheme + `My Mac` destination
3. ⌘R（Xcode 自动用你的 Apple Developer 签名）

**第一次开 VPN 开关时**：
1. macOS 弹「VPN-macOS 想要添加 VPN 配置 / 输入密码」→ 输 Mac 登录密码 → 允许
2. 如果没弹密码框就直接报错 → 去 `系统设置 → 隐私与安全性`，**最下面**有「VPN 配置已被阻止」红字 → 点「允许」
3. 再开一次开关 → 应该就通了

> 现在的版本（W3 起）**不再用 App Group**，不会再弹「VPN-macOS 想访问其他 App 数据」的隐私警告。xray JSON 通过 `providerConfiguration` 直接传给 Tunnel Extension；geo 数据库内嵌在 Extension Resources 里。
>
...
```bash
cd /Users/sbraveyoung/code/github.com/sbraveyoung/vpn/Apps
xcodegen generate
open VPN.xcodeproj
```
...
- AppGroup entitlement 没生效。在 Apple Developer 后台确认 `group.com.sbraveyoung.vpn` 已绑定到 4 个 Bundle ID 都做过 Configure。
...
- xray-core 启动可能在阻塞主线程。在 Console.app 过滤 `VPN-Tunnel-iOS` 看 stack。
...
2. 过滤 `com.sbraveyoung.vpn` 看主 App + Tunnel 两个进程的日志混合输出

**macOS：**
```bash
log stream --predicate 'subsystem CONTAINS "com.sbraveyoung.vpn"' --info
```
...
⚠️ **失败路径**：哪一步炸了 + Xcode 控制台**最近的 20 行日志**截图（必须含 `com.sbraveyoung.vpn` 相关行）
...
```bash
codesign -dv /Applications/VPN-macOS.app 2>&1 | head -5
```
...
`系统设置 → 网络` 看左边栏有没有 `VPN-macOS` 条目：

| 看到 | 含义 | 怎么办 |
|---|---|---|
| 没有任何 VPN 条目 | 配置保存失败 | 看上面的「VPN 配置已被阻止」 |
| 有 `VPN-macOS` 但显示「未启用」 | 配置存了但被禁 | 点进去 toggle 开 |
| 有 `VPN-macOS` 显示「正在连接」一直转 | xray-core 自己有问题 | 看下面「xray-core 启动失败」 |

### 3. 系统设置里看是否被拦截

`系统设置 → 隐私与安全性`，往下拉到最底：

- 如果有红色 / 黄色字 **「VPN 配置已被阻止 / VPN-macOS was blocked from adding a VPN configuration」** → 点右边「允许」
- 如果有 **「来自身份不明的开发者」** → 也点允许（虽然你真签了，但 entitlement 第一次仍会触发审查）

### 4. 看实时日志看真实错误

```bash
log stream --predicate 'subsystem CONTAINS "com.sbraveyoung.vpn" OR subsystem CONTAINS "NetworkExtension"' --info 2>&1 | head -50
```
...
# 删 app 然后重装
rm -rf /Applications/VPN-macOS.app
./install.sh --signed
```
